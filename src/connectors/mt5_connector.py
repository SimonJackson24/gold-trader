...
class MT5Connector:
    ...
    async def connect(self):
        ...
        if getattr(self.settings, "dev_mock_mt5", False):
            ...
            try:
                ...
                self.logger.warning(
                    f"Could not connect to local websocket in mock mode: {e}"
                )
            return
    ...
    async def _handle_trade_request(self, request_data: dict):
        ...
        try:
            ...
            if self.websocket:
                await self.websocket.send(
                    json.dumps({"type": "trade_result", "data": result})
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {"symbol": symbol, "subscribed": subscribe},
                        }
                    )
                )
        ...
        while symbol in self.subscribed_symbols and self.is_connected:
            try:
                ...
                if self.websocket:
                    try:
                        await self.websocket.send(
                            json.dumps({"type": "tick", "data": tick_data.to_dict()})
                        )
                    except Exception as e:
