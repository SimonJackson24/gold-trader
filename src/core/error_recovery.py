...
class CircuitBreaker:
    ...
    def _on_failure(self):
        ...
        if self.state.failure_count >= self.failure_threshold:
            ...
            self.logger.warning(f"Circuit breaker OPEN after {self.state.failure_count} failures")
    ...
    async def execute(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self.logger.warning(f"Retry {attempt + 1}/{self.max_retries} failed: {e}, retrying in {delay:.2f}s")
...
class ErrorRecoveryManager:
    ...
    def _register_default_handlers(self):
        ...
        self.register_error_handler("authentication_error", self._handle_authentication_error)
    ...
    def register_retry_handler(self, name: str, max_retries: int = 3,
                           base_delay: float = 1.0) -> RetryHandler:
        """
...
def with_circuit_breaker(breaker_name: str, failure_threshold: int = 5, recovery_timeout: int = 60):
    """
