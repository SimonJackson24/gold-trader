...
class Trade:
    ...
    def __post_init__(self):
        ...
        if self.position_size < 0:
            raise ValueError(
                f"Position size cannot be negative, got {self.position_size}"
            )
    ...
    def current_profit_loss(self) -> Optional[Decimal]:
        ...
        return (
            self.current_pips * self.position_size * Decimal("10")
        )  # Assuming $10 per pip per lot
    ...
    def _calculate_current_pl(self):
        ...
        self.profit_loss = (
            self.profit_loss_pips * self.remaining_position_size * Decimal("10")
        )

        # Calculate percentage
        if self.position_size > 0:
            self.profit_loss_percentage = (
                self.profit_loss / (self.position_size * Decimal("1000"))
            ) * 100
    ...
    def partial_close(self, size: Decimal, price: Decimal, reason: str):
        ...
        if size > self.remaining_position_size:
            raise ValueError(
                f"Cannot close {size}, remaining position is {self.remaining_position_size}"
            )
        ...
        partial = PartialClose(
            time=datetime.utcnow(), price=price, size=size, profit=profit, reason=reason
        )
    ...
    def from_dict(cls, data: dict) -> "Trade":
        ...
        exit_reason = (
            ...
            if data.get("exit_reason") and isinstance(data["exit_reason"], str)
            else data.get("exit_reason")
        )
