...
class Candle:
    ...
    def __post_init__(self):
        ...
        if not (self.low <= self.open <= self.high):
            raise ValueError(f"Open price ({self.open}) must be between high ({self.high}) and low ({self.low})")
        
        if not (self.low <= self.close <= self.high):
            raise ValueError(f"Close price ({self.close}) must be between high ({self.high}) and low ({self.low})")
    ...
    def is_engulfing(self, previous_candle: 'Candle') -> bool:
        ...
        # Bullish engulfing
        if (previous_candle.is_bearish and self.is_bullish and
            self.open < previous_candle.close and
            self.close > previous_candle.open):
            return True
        
        # Bearish engulfing
        if (previous_candle.is_bullish and self.is_bearish and
            self.open > previous_candle.close and
            self.close < previous_candle.open):
            return True
