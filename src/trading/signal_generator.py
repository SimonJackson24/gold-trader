...
class SignalGenerator:
    ...
    async def generate_signal(
        self, analysis: ConfluenceAnalysis, current_price: Decimal
        ...
        direction, setup_type = self._determine_signal_direction(
        ...
        signal = TradingSignal(
            ...
            + timedelta(minutes=self.settings.trading.signal_expiry_minutes),
    ...
    def _calculate_entry_price(
        self, analysis: ConfluenceAnalysis, current_price: Decimal, direction: str
    ) -> Decimal:
        """
    ...
    def _calculate_price_levels(
        self, entry_price: Decimal, direction: str, analysis: ConfluenceAnalysis
        ...
        if direction == "BUY":
            ...
            tp1 = (
                entry_price + (entry_price - stop_loss) * trading_config.tp1_percentage
            )
            tp2 = (
                entry_price + (entry_price - stop_loss) * trading_config.tp2_percentage
            )
        else:  # SELL signal: SL above entry
            stop_loss = entry_price + sl_buffer_pips
            tp1 = (
                entry_price - (stop_loss - entry_price) * trading_config.tp1_percentage
            )
            tp2 = (
                entry_price - (stop_loss - entry_price) * trading_config.tp2_percentage
            )

        return stop_loss, tp1, tp2

    def _calculate_position_risk(
        self, entry_price: Decimal, stop_loss: Decimal, direction: str
    ) -> tuple[Decimal, float]:
        """
        ...
        position_size = trading_config.calculate_position_size(
            trading_config.get_risk_amount(), risk_pips
        )
    ...
    def _calculate_risk_reward(
        self, entry_price: Decimal, stop_loss: Decimal, tp2: Decimal, direction: str
    ) -> float:
        """
        Calculate risk:reward ratio.
    ...
    def _create_context_description(self, timeframe_analysis) -> str:
        ...
        # Get top factors
        top_factors = sorted(
            timeframe_analysis.factors, key=lambda f: f.score, reverse=True
        )[:3]
