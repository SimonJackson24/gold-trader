...
class TradeManager:
        ...
        try:
            ...
            async with self._trade_semaphore.acquire(timeout=10.0):
                ...
                if not lock_acquired:
                    self.logger.warning(f"Failed to acquire lock for trade {signal.signal_id}")
                    return None
                ...
                try:
                    ...
                    await self._trade_lock.release_resource(f"trade_{signal.signal_id}", "trade_manager")
    ...
    async def _can_open_trade(self) -> bool:
        ...
        active_count = len([
            resource_id for resource_id in locked_resources.keys()
                ...
                try:
                    ...
                    if lock_acquired:
                        try:
                            ...
                            await self._trade_lock.release_resource(resource_id, "monitor")
    ...
    async def _update_trade_price(self, trade_id: int, current_price: Decimal):
        ...
        try:
            ...
            if not lock_acquired:
                self.logger.warning(f"Failed to acquire lock for trade {trade_id} price update")
                return
        ...
        try:
            ...
            try:
                ...
                if not trade.tp1_hit and trade.is_price_at_risk_reward_level(
                    ...
                    await self._partial_close_trade(trade_id, trade.take_profit_1, 0.5, "TP1_HIT")
                    return
                ...
                if (not trade.breakeven_moved and
                    self.settings.trading.move_to_breakeven_pips > 0):
                    move_to_be_pips = self.settings.trading.move_to_breakeven_pips / 10000
                    
                    if trade.is_buy and current_price >= (trade.entry_price + move_to_be_pips):
                        await self.trade_repo.update_trade_price(trade_id, trade.entry_price)
                        trade.move_stop_to_breakeven()
                        self.logger.info(f"Trade {trade_id} moved to breakeven")
                    elif trade.is_sell and current_price <= (trade.entry_price - move_to_be_pips):
                        await self.trade_repo.update_trade_price(trade_id, trade.entry_price)
                        trade.move_stop_to_breakeven()
                        self.logger.info(f"Trade {trade_id} moved to breakeven")
    ...
    async def _partial_close_trade(self, trade: Trade, close_price: Decimal, 
        ...
        try:
            ...
            # Save to database
            await self.trade_repo.close_trade(trade.trade_id, close_price, datetime.utcnow(), reason)
    ...
    async def _close_trade(self, trade: Trade, close_price: Decimal, 
                       close_time: datetime, reason: ExitReason):
        """
        ...
        try:
            ...
            # Save to database
            await self.trade_repo.close_trade(trade.trade_id, close_price, close_time, reason)
            ...
            self.logger.info(
                f"Trade closed: {trade.trade_id} with P/L {profit}"
            )
    ...
    async def close_trade_manually(self, trade_id: int, reason: str = "MANUAL_CLOSE") -> bool:
        """
        ...
        await self._close_trade(trade, current_price, datetime.utcnow(), ExitReason(reason))
        return True
