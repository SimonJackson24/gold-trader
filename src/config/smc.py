...
class FVGConfig(BaseSettings):
    ...
    fill_timeframe_minutes: int = Field(
        default=60, ge=15, le=240, env="FVG_FILL_TIMEFRAME"
    )
    fill_threshold_percentage: float = Field(
        default=0.7, ge=0.5, le=1.0, env="FVG_FILL_THRESHOLD"
    )
    ...
    volume_multiplier: float = Field(
        default=1.5, ge=1.0, le=5.0, env="FVG_VOLUME_MULTIPLIER"
    )
...
class OrderBlockConfig(BaseSettings):
    ...
    wick_ratio_threshold: float = Field(
        default=0.6, ge=0.3, le=0.9, env="OB_WICK_RATIO"
    )

    # Volume requirements
    min_volume_multiplier: float = Field(
        default=1.5, ge=1.0, le=5.0, env="OB_VOLUME_MULTIPLIER"
    )
    avg_volume_periods: int = Field(
        default=20, ge=5, le=100, env="OB_AVG_VOLUME_PERIODS"
    )

    # Validation
    require_rejection: bool = Field(default=True, env="OB_REQUIRE_REJECTION")
    rejection_ratio: float = Field(
        default=0.4, ge=0.2, le=0.8, env="OB_REJECTION_RATIO"
    )
    ...
    round_number_pips: int = Field(
        default=50, ge=10, le=200, env="OB_ROUND_NUMBER_PIPS"
    )
...
class LiquidityConfig(BaseSettings):
    ...
    swing_point_threshold: float = Field(
        default=0.5, ge=0.2, le=2.0, env="LIQ_SWING_THRESHOLD"
    )

    # Sweep detection
    sweep_extension_pips: int = Field(default=5, ge=1, le=20, env="LIQ_SWEEP_EXTENSION")
    reversal_threshold: float = Field(
        default=0.7, ge=0.3, le=1.0, env="LIQ_REVERSAL_THRESHOLD"
    )
    reversal_timeframe_minutes: int = Field(
        default=30, ge=5, le=120, env="LIQ_REVERSAL_TIMEFRAME"
    )

    # Volume analysis
    volume_spike_multiplier: float = Field(
        default=2.0, ge=1.0, le=5.0, env="LIQ_VOLUME_SPIKE"
    )
    avg_volume_period: int = Field(
        default=20, ge=5, le=100, env="LIQ_AVG_VOLUME_PERIOD"
    )
...
class StructureConfig(BaseSettings):
    ...
    trend_strength_threshold: float = Field(
        default=0.6, ge=0.3, le=1.0, env="STRUCT_TREND_STRENGTH"
    )

    # Structure identification
    min_swing_points: int = Field(default=3, ge=2, le=10, env="STRUCT_MIN_SWING_POINTS")
    structure_break_threshold: float = Field(
        default=0.8, ge=0.5, le=1.0, env="STRUCT_BREAK_THRESHOLD"
    )

    # BOS and CHoCH
    bos_confirmation_candles: int = Field(
        default=2, ge=1, le=5, env="STRUCT_BOS_CONFIRMATION"
    )
    choch_confirmation_candles: int = Field(
        default=1, ge=1, le=3, env="STRUCT_CHOCH_CONFIRMATION"
    )

    # Range detection
    range_threshold_percentage: float = Field(
        default=0.3, ge=0.1, le=0.8, env="STRUCT_RANGE_THRESHOLD"
    )
    range_min_periods: int = Field(
        default=10, ge=5, le=50, env="STRUCT_RANGE_MIN_PERIODS"
    )
...
class SMCConfig(BaseSettings):
    ...
    confluence_threshold: float = Field(
        default=80.0, ge=50.0, le=100.0, env="SMC_CONFLUENCE_THRESHOLD"
    )
    ...
    liquidity_weight: float = Field(
        default=25.0, ge=0.0, le=100.0, env="SMC_LIQUIDITY_WEIGHT"
    )
    structure_weight: float = Field(
        default=20.0, ge=0.0, le=100.0, env="SMC_STRUCTURE_WEIGHT"
    )
    ...
    min_candles_required: int = Field(
        default=200, ge=50, le=1000, env="SMC_MIN_CANDLES"
    )
    analysis_buffer_candles: int = Field(
        default=20, ge=5, le=100, env="SMC_ANALYSIS_BUFFER"
    )
    ...
    def validate(self) -> bool:
        ...
        if errors:
            raise ValueError(
                "SMC configuration validation failed:\n"
    ...
    def calculate_confluence_score(self, analysis_results: dict) -> float:
        ...
        liquidity_score = analysis_results.get("liquidity_score", 0) * (
            self.liquidity_weight / 100.0
        )
        structure_score = analysis_results.get("structure_score", 0) * (
            self.structure_weight / 100.0
        )
