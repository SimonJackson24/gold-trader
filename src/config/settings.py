...
class Settings(BaseSettings):
    ...
    jwt_expire_minutes: int = Field(
        default=60, env="JWT_EXPIRE_MINUTES"
    )  # 1 hour for access tokens
    jwt_refresh_expire_minutes: int = Field(
        default=1440, env="JWT_REFRESH_EXPIRE_MINUTES"
    )  # 24 hours for refresh tokens
    jwt_refresh_token_enabled: bool = Field(
        default=True, env="JWT_REFRESH_TOKEN_ENABLED"
    )
    ...
    def validate_configuration(self) -> bool:
        ...
        if not (1 <= self.ws_port <= 65535):
            errors.append(
                f"WebSocket port {self.ws_port} is out of valid range (1-65535)"
            )
