...
class TradingConfig(BaseSettings):
    ...
    max_concurrent_trades: int = Field(default=2, ge=1, le=10, env="MAX_CONCURRENT_TRADES")
    min_risk_reward: float = Field(default=2.0, ge=1.5, le=10.0, env="MIN_RISK_REWARD")
    
    # Position sizing
    default_lot_size: float = Field(default=0.01, ge=0.01, le=10.0, env="DEFAULT_LOT_SIZE")
    ...
    entry_timeout_minutes: int = Field(default=30, ge=5, le=240, env="ENTRY_TIMEOUT_MINUTES")
    signal_expiry_minutes: int = Field(default=240, ge=60, le=1440, env="SIGNAL_EXPIRY_MINUTES")
    ...
    ny_overlap_session: SessionConfig = Field(default_factory=lambda: SessionConfig(
    ...
    news_filter_minutes: int = Field(default=30, ge=0, le=120, env="NEWS_FILTER_MINUTES")
    ...
    def validate(self) -> bool:
        ...
        if errors:
            raise ValueError("Trading configuration validation failed:\n" + "\n".join(f"- {error}" for error in errors))
    ...
    def calculate_position_size(self, risk_amount: float, stop_loss_pips: Decimal) -> Decimal:
        """
