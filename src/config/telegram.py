...
class TelegramConfig(BaseSettings):
    ...
    channel_username: Optional[str] = Field(
        default=None, env="TELEGRAM_CHANNEL_USERNAME"
    )
    ...
    enable_trade_updates: bool = Field(
        default=True, env="TELEGRAM_ENABLE_TRADE_UPDATES"
    )
    ...
    messages_per_minute: int = Field(
        default=5, ge=1, le=30, env="TELEGRAM_MESSAGES_PER_MINUTE"
    )
    ...
    connection_timeout: int = Field(
        default=30, ge=5, le=120, env="TELEGRAM_CONNECTION_TIMEOUT"
    )
    ...
    def validate(self) -> bool:
        ...
        if self.burst_limit < self.messages_per_minute:
            errors.append(
                "Burst limit should be greater than or equal to messages per minute"
            )
    ...
    def create_progress_bar(
        self, current_price: float, entry_price: float, tp1: float, tp2: float
    ) -> str:
        """
