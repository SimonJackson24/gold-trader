...
class Signal(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "confidence_score >= 0 AND confidence_score <= 1", name="check_confidence"
        ),
        CheckConstraint(
...
class Trade(Base):
    ...
    # Foreign key
    signal_id = Column(
        String(50), ForeignKey("signals.signal_id", ondelete="CASCADE"), nullable=True
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('PENDING', 'OPEN', 'CLOSED', 'CANCELLED', 'STOPPED_OUT')",
...
class PerformanceMetric(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "instrument", "metric_date", name="uq_performance_instrument_date"
        ),
...
class PriceHistory(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "timeframe IN ('M1', 'M5', 'M15', 'M30', 'H1', 'H4', 'D1')",
            name="check_timeframe",
        ),
        UniqueConstraint(
            "instrument", "timeframe", "timestamp", name="uq_price_history"
        ),
        Index(
            "idx_price_history_instrument_timeframe_timestamp",
            "instrument",
            "timeframe",
            "timestamp",
        ),
