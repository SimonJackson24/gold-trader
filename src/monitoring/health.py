...
class HealthChecker:
        ...
        for check in checks:
            try:
                ...
                result = await asyncio.wait_for(
                    self._run_check(check), timeout=check.timeout
                )
                ...
                self.logger.debug(
                    f"Health check {check.name}: {result.status.value} ({duration:.3f}s)"
                )
    ...
    async def _run_check(self, check: HealthCheck) -> HealthResult:
        ...
        try:
            ...
            return HealthResult(
                name=check.name, status=status, message=message, duration=0.0
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_checks": len(
                [r for r in self.results.values() if r.status == HealthStatus.UNHEALTHY]
            ),
        }
...
def setup_health_checks(
    mt5_connector=None, websocket_server=None, telegram_service=None
):
