...
class OrderBlock:
    ...
    def calculate_strength(
        self, avg_volume: float, volume_profile: dict = None
    ) -> float:
        """
    ...
    def is_valid(self, min_strength: float = 0.3) -> bool:
        ...
        return (
            ...
            and self.wick_ratio <= config.wick_ratio_threshold
            and self.range_size >= config.min_candle_range
        )
...
class OrderBlockDetector:
    ...
    def detect_order_blocks(
        self, candles: List[Candle], swing_points: List[SwingPoint] = None
    ) -> List[OrderBlock]:
        """
    ...
    def _is_potential_order_block(
        self, candle: Candle, candles: List[Candle], index: int
    ) -> bool:
        """
        ...
        return (
            self._has_significant_range(candle)
            and self._has_volume_spike(candle)
            and self._shows_accumulation(candle, prev_candle, next_candle)
        )
    ...
    def _shows_accumulation(
        self, candle: Candle, prev_candle: Candle, next_candle: Candle
    ) -> bool:
        """
        ...
        body_overlap = min(candle.high, prev_candle.high) >= max(
            candle.low, prev_candle.low
        )

        return (
            candle.body_percentage <= 40  # Small body
            and body_overlap  # Overlapping ranges
            and abs(candle.close - prev_candle.close) < candle.total_range * 0.3
        )
    ...
    def _determine_block_type(
        ...
        future_candles = (
            candles[index + 1 : index + 4] if index + 4 < len(candles) else []
        )
    ...
    def validate_order_block_respect(
        self, order_block: OrderBlock, current_price: Decimal
    ) -> bool:
        """
    ...
    def get_order_block_statistics(self, order_blocks: List[OrderBlock]) -> dict:
        ...
        return {
            ...
            "weakest_block": min(order_blocks, key=lambda x: x.strength)
            if order_blocks
            else None,
        }
