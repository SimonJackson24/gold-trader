...
class StructureBreak:
    ...
    def __init__(self, break_type: str, price: Decimal, timestamp: datetime,
                 confirmed: bool = False, strength: float = 0.0):
        """
...
class MarketStructure:
    ...
    def _update_trend_analysis(self):
        ...
        if len(self.swing_highs) < self.min_swing_points or len(self.swing_lows) < self.min_swing_points:
            self.current_state = MarketStructureState.RANGING
            self.trend_direction = None
            self.trend_strength = 0.0
            return
    ...
    def _calculate_trend_strength(self, highs: List[SwingPoint], 
                               lows: List[SwingPoint]) -> float:
        """
    ...
    def confirm_structure_break(self, break_type: str, price: Decimal):
        ...
        for structure_break in self.structure_breaks:
            if (structure_break.type == break_type and 
                abs(structure_break.price - price) < Decimal('0.0010')):
    ...
    def get_recent_breaks(self, max_age_hours: int = 24) -> List[StructureBreak]:
        ...
        recent_breaks = [
            sb for sb in self.structure_breaks
            if sb.timestamp >= cutoff_time
        ]
