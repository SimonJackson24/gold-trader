...
class ConfluenceFactor:
    ...
    def __init__(
        self, factor_type: str, score: float, description: str, weight: float = 1.0
    ):
        """
...
class ConfluenceAnalysis:
    ...
    def analyze(
        ...
        # Detect Order Blocks
        swing_points = (
            self.market_structure_obj.swing_highs + self.market_structure_obj.swing_lows
        )
    ...
    def _determine_setup_type(self):
        ...
        liquidity_present = any(
            f.type.startswith("LIQUIDITY") for f in self.h4_analysis.factors
        )
...
class ConfluenceAnalyzer:
    ...
    def calculate_signal_quality(
        self, analysis: ConfluenceAnalysis
    ) -> Dict[str, float]:
        """
        ...
        if analysis.m15_analysis.factors:
            ...
            precision_factors = [
                f
                for f in analysis.m15_analysis.factors
                if f.type
                in ["BULLISH_CONFIRMATION", "BEARISH_CONFIRMATION", "PRICE_REJECTION"]
            ]
    ...
    def validate_signal_setup(self, analysis: ConfluenceAnalysis) -> Dict[str, Any]:
        ...
        if (
            self.config.require_multi_timeframe
            and analysis.h4_analysis.overall_score < 50
            and analysis.h1_analysis.overall_score < 50
            and analysis.m15_analysis.overall_score < 50
        ):
            validation["is_valid"] = False
            validation["errors"].append("Insufficient multi-timeframe alignment")

        # Check for conflicting signals
        if self._has_conflicting_factors(analysis):
            validation["warnings"].append("Potential conflicting factors detected")
    ...
    def _has_conflicting_factors(self, analysis: ConfluenceAnalysis) -> bool:
        ...
        has_bullish = any(
            "BULLISH" in f.description.upper() for f in analysis.h4_analysis.factors
        )
        has_bearish = any(
            "BEARISH" in f.description.upper() for f in analysis.h4_analysis.factors
        )
