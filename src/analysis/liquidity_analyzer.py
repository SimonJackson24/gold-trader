...
class LiquidityPool:
    ...
    def __init__(self, price: Decimal, strength: float, pool_type: str,
                 timestamp: datetime, instrument: str = "XAUUSD"):
        """
    ...
    def _validate_pool(self):
        ...
        if self.strength < 0 or self.strength > 1:
            raise ValueError(f"Pool strength must be between 0 and 1, got {self.strength}")
    ...
    def is_within_range(self, price: Decimal, tolerance_pips: Decimal = Decimal('5')) -> bool:
        """
...
class LiquiditySweep:
    ...
    def __init__(self, sweep_type: str, pool_price: Decimal, sweep_price: Decimal,
                 sweep_time: datetime, reversal_price: Optional[Decimal] = None):
        """
    ...
    def calculate_strength(self, volume_at_sweep: float, avg_volume: float) -> float:
        ...
        if avg_volume > 0:
            ...
            volume_strength = min(1.0, volume_multiplier / config.volume_spike_multiplier)
        
        # Extension-based strength (optimal extension is 5-15 pips)
        extension_pips = float(self.extension_pips * 10000)
        if config.sweep_extension_pips <= extension_pips <= config.sweep_extension_pips * 3:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'reversal_price': float(self.reversal_price) if self.reversal_price else None,
            'strength': self.strength,
...
class LiquidityAnalyzer:
    ...
    def identify_liquidity_pools(self, swing_points: List[SwingPoint],
                                   current_price: Decimal) -> List[LiquidityPool]:
        """
        ...
        for i in range(len(swing_points) - 1):
            ...
            if self._forms_liquidity_pool(current_sp, next_sp, current_price):
                pool = LiquidityPool(
                    ...
                    pool_type=self._determine_pool_type(current_sp, next_sp, current_price),
    ...
    def _forms_liquidity_pool(self, sp1: SwingPoint, sp2: SwingPoint,
        ...
        near_pool = (
            abs(current_price - sp1.price) <= tolerance or
            abs(current_price - sp2.price) <= tolerance
        )
    ...
    def _calculate_pool_strength(self, sp1: SwingPoint, sp2: SwingPoint) -> float:
        ...
        touches_strength = min(1.0, (sp1.touches + sp2.touches) / (self.config.min_pool_touches * 2))
        ...
        # Combine strengths
        total_strength = (touches_strength * 0.4) + (recency_strength * 0.3) + (point_strength * 0.3)
        
        return min(1.0, max(0.0, total_strength))
    
    def _determine_pool_type(self, sp1: SwingPoint, sp2: SwingPoint,
    ...
    def detect_liquidity_sweeps(self, pools: List[LiquidityPool], 
                               candles: List[Candle], current_price: Decimal) -> List[LiquiditySweep]:
        """
    ...
    def _analyze_sweep_pattern(self, pool: LiquidityPool, 
                           candles: List[Candle], current_price: Decimal) -> Optional[LiquiditySweep]:
        """
    ...
    def analyze_liquidity_flow(self, pools: List[LiquidityPool], 
                             sweeps: List[LiquiditySweep], current_price: Decimal) -> dict:
        """
        ...
        return {
            ...
            'sweep_rate': (confirmed_sweeps / total_sweeps * 100) if total_sweeps > 0 else 0,
    ...
    def _calculate_liquidity_score(self, pools: List[LiquidityPool], 
