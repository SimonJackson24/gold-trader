...
from src.core import (
    trade_locks, signal_cache, trade_cache,
    error_recovery_manager, memory_monitor,
    security_logger, audit_logger
)
...
class TestSignalGeneration:
    ...
    def sample_confluence_analysis(self):
        ...
        mock_analysis.get_confluence_factors.return_value = [
            "FVG", "ORDER_BLOCK", "BULLISH_CONFIRMATION"
        ]
    ...
    @pytest.mark.asyncio
    async def test_signal_generation_success(self, signal_generator, sample_confluence_analysis):
...
class TestTradeExecution:
    ...
    async def test_trade_monitoring_price_update(self, trade_manager):
        ...
        with patch.object(trade_manager, '_get_current_price') as mock_price:
            ...
            with patch.object(trade_manager.trade_repo, 'update_trade_price') as mock_update:
...
class TestRiskManagement:
    ...
    def test_risk_percentage_validation(self, trade_manager):
        ...
        valid_risk = 2.0
        assert trade_manager.settings.trading.validate_risk_percentage(valid_risk) is True
        
        # Test invalid risk percentage
        invalid_risk = 10.0  # Too high
        assert trade_manager.settings.trading.validate_risk_percentage(invalid_risk) is False
...
class TestSecurityScenarios:
    ...
    async def test_rate_limiting(self):
        ...
        for i in range(10):
            allowed = await jwt_handler.check_rate_limit(mock_request, limit=20, window=60)
...
class TestPerformanceAndMemory:
    ...
    async def test_resource_locking(self):
        ...
        # Acquire lock
        acquired = await resource_manager.acquire_resource("test_resource", "write", "test_user")
        assert acquired is True
        
        # Try to acquire same lock (should fail)
        acquired2 = await resource_manager.acquire_resource("test_resource", "write", "test_user2")
        ...
        acquired3 = await resource_manager.acquire_resource("test_resource", "write", "test_user3")
