...
class TestAnalysisEndpoints:
    ...
    def test_smart_money_analysis_with_auth(self, mock_settings, sample_analysis_result):
        """Test Smart Money analysis with authentication."""
        with patch('src.api.main.smart_money_engine') as mock_sme, \
            ...
            with TestClient(app) as client:
                headers = {"Authorization": "Bearer test_secret_key"}
                response = client.post("/analysis/smart-money?symbol=XAUUSD", headers=headers)
                assert response.status_code == 200
...
class TestWebSocketEndpoint:
    ...
    def test_websocket_subscribe_with_auth(self, mock_settings):
        ...
        with TestClient(app) as client:
            headers = {"Authorization": "Bearer test_secret_key"}
            response = client.post("/websocket/subscribe", json=subscription_data, headers=headers)
            assert response.status_code == 200
    ...
    def test_websocket_subscribe_invalid_channels(self, mock_settings):
        ...
        with TestClient(app) as client:
            headers = {"Authorization": "Bearer test_secret_key"}
            response = client.post("/websocket/subscribe", json=invalid_data, headers=headers)
            assert response.status_code == 422  # Validation error
